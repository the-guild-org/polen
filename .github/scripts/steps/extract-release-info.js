 ...
 */
export default async ({ core, semver }) => {
  try {
    ...
    const action = isWorkflowDispatch ? 'manual' : process.env.GITHUB_EVENT_ACTION
    ...
    if (tag === 'next' || tag === 'latest') {
      ...
      if (tag === 'next' && action === 'edited') {
        try {
          ...
          const allTags = execSync(`git tag --points-at ${commit}`, { encoding: 'utf-8' })
            .split('\n')
            .filter(t => t.trim())

          const semverTag = allTags.find(t => semver.valid(t) && semver.prerelease(t))
    ...
    try {
      const demoConfig = JSON.parse(readFileSync('.github/demo-config.json', 'utf-8'))
      const minVersion = demoConfig.minimumVersion || '0.0.0'

      if (semver.gte(tag, minVersion)) {
        console.log(`✅ Version ${tag} meets minimum requirement (${minVersion})`)
        core.setOutput('needs_build', 'true')
      } else {
        console.log(`❌ Version ${tag} is below minimum demo version ${minVersion}`)
        console.log(`⚠️  Demos are only supported for versions ${minVersion} and above`)
        core.setOutput('needs_build', 'false')
      }
    } catch (e) {
