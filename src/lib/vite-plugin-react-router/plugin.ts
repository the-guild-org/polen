...
const PKG_NAME = import.meta.dirname
...
export function reactRouter(options?: ReactRouterOptions): Plugin[] {
  ...
  return [
    {
      ...
      resolveId(source) {
        // console.log(`[RR Plugin] rsolveId`, this.environment?.name, source)
      ...
      async load(id) {
        if (id === '\0virtual:react-router-routes') {
          // console.log(`[RR Plugin] Loading virtual:react-router-routes in environment: ${this.environment?.name}`)
          const findFile = (id: string) => idResolver(this.environment, id)
          const config = await readReactRouterConfig(findFile, { environment: this.environment, ...options })
          // dump(config.routes)
...
async function readReactRouterConfig(
  ...
  options?: ReactRouterOptions & { environment?: Environment | undefined },
) {
  ...
  if (configFile) {
    ...
    appDirectory = path.resolve(
      configImport.module.default.appDirectory ?? options?.appDirectory ?? 'app',
    )
  } else {
    // Use provided options or default
    appDirectory = path.resolve(options?.appDirectory ?? 'app')
  }

  // find routes.ts
  const routesFile = await findFile(path.join(appDirectory, 'routes'))
  assert(routesFile, "Cannot find 'routes' file")
  // console.log(`[RR Plugin] About to runnerImport routes file: ${routesFile}`)

  let routesModule: {
    default: RouteConfigEntry[]
  }
  if (options?.environment && isRunnableDevEnvironment(options.environment)) {
    routesModule = await options.environment.runner.import<{
      default: RouteConfigEntry[]
    }>(routesFile)
  } else {
  ...
  const routes = [
    {
      ...
      children: routesModule.default,
    },
  ]

  return { configFile: configFile || undefined, routesFile, appDirectory, routes }
}
