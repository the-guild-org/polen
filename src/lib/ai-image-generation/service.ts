...
export class AiImageGenerationService {
   ...
   */
  async generate(options: GenerateOptions): Promise<GeneratedImage | null> {
   ...
   */
  private async cacheImage(image: GeneratedImage): Promise<void> {
    try {
      const cacheKey = this.getCacheKey(image.prompt)
      const cachePath = Path.join(this.cacheDir, `${cacheKey}.json`)
      ...
      // Write cache file
      await FS.writeFile(cachePath, JSON.stringify(image, null, 2))
    } catch (error) {
