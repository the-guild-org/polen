 ...
 */
export async function build(tag: string, options: BuildOptions = {}): Promise<void> {
  const basePath = options.basePath || `/polen/${tag}/`
  const examples = options.examples || await getDemoExamples()

  for (const example of examples) {
    await buildExample(example, `${basePath}${example}/`, options.outputDir)
  }
}
...
export async function deploy(options: DeployOptions): Promise<void> {
  ...
  for (const example of examples) {
    ...
    try {
      ...
      await copyDirectory(buildDir, destDir)
  ...
  if (updateBasePaths) {
    await updateBasePathsInFiles(targetDir, updateBasePaths.from, updateBasePaths.to)
 ...
 */
export async function updateBasePathsInFiles(
  dir: string,
  oldPath: string,
  newPath: string,
): Promise<void> {
 ...
 *
 * This function automates the process of building demos for multiple Polen versions:
 *
 ...
 * ```
 *
 * Note: This function modifies the git working directory by checking out tags.
 * The original branch is restored by default to prevent issues in CI environments.
 */
export async function buildMultipleVersions(
  versions: string[],
  deployDir: string,
  ...
  try {
    for (const version of versions) {
      ...
      if (!meetsMinimumPolenVersion(config, version)) {
        console.log(
          `⚠️ Skipping ${version} - below minimum Polen version ${config.examples.minimumVersion}`,
        )
 ...
 */
export async function buildCurrentCycle(deployDir: string): Promise<void> {
  ...
  await buildMultipleVersions(versions, deployDir)
}
...
export const demoBuilder = {
  build,
  deploy,
  updateBasePaths: updateBasePathsInFiles,
