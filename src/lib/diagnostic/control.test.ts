...
import { Control, getEffetivePhaseSettings } from './control.js'
...
describe('getPhaseSettings', () => {
  ...
  test.for([
    ...
    expect(getEffetivePhaseSettings(control, 'dev', defaults)).toEqual(expected)
  })
  ...
  test.for([
    ...
    expect(getEffetivePhaseSettings(Control.make(control), phase, defaults)).toEqual(expected)
  })
  ...
  test('different phases can have different severities', () => {
    ...
    expect(getEffetivePhaseSettings(control, 'dev', defaults)).toEqual({ enabled: true, severity: 'info' })
    expect(getEffetivePhaseSettings(control, 'build', defaults)).toEqual({ enabled: true, severity: 'error' })
  })
})
