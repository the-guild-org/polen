var e = `Cypher`,
  ...
  i = {
    ...
    constants: {
      patterns: [{ match: `(?i)\\bTRUE|FALSE\\b`, name: `constant.language.bool.cypher` }, {
        match: `(?i)\\bNULL\\b`,
    ...
    functions: {
      patterns: [{
        ...
        comment: `List of Cypher built-in functions from http://docs.neo4j.org/chunked/milestone/query-function.html`,
        match:
          `(?i)\\b(ABS|ACOS|ASIN|ATAN|ATAN2|COS|COT|DEGREES|E|EXP|FLOOR|HAVERSIN|LOG|LOG10|PI|RADIANS|RAND|ROUND|SIGN|SIN|SQRT|TAN)(?=\\s*\\()`,
    ...
    identifiers: {
      patterns: [{ match: '`.+?`', name: `variable.other.quoted-identifier.cypher` }, {
    ...
    keywords: {
      patterns: [
        {
          match:
            `(?i)\\b(START|MATCH|WHERE|RETURN|UNION|FOREACH|WITH|AS|LIMIT|SKIP|UNWIND|HAS|DISTINCT|OPTIONAL\\\\s+MATCH|ORDER\\s+BY|CALL|YIELD)\\b`,
          name: `keyword.control.clause.cypher`,
        },
        { match: `(?i)\\b(ELSE|END|THEN|CASE|WHEN)\\b`, name: `keyword.control.case.cypher` },
        {
        ...
        {
          match:
            `(?i)\\b(USING\\s+INDEX|CREATE\\s+INDEX\\s+ON|DROP\\s+INDEX\\s+ON|CREATE\\s+CONSTRAINT\\s+ON|DROP\\s+CONSTRAINT\\s+ON)\\b`,
        ...
        { match: `(?i)\\b(DESC|ASC)\\b`, name: `keyword.other.order.cypher` },
        {
          ...
          patterns: [
            { match: '((?:`.+?`)|(?:[\\p{L}_][\\p{L}0-9_]*))', name: `variable.parameter.relationship-name.cypher` },
            { match: `(\\*)`, name: `keyword.control.starting-function-params.cypher` },
            { include: `#comments` },
            { include: `#numbers` },
            { include: `#strings` },
          ],
    ...
    numbers: { patterns: [{ match: `\\b\\d+(\\.\\d+)?\\b`, name: `constant.numeric.cypher` }] },
    operators: {
      patterns: [
        { match: `(\\+|\\-|\\/|\\*|\\%|\\?|!)`, name: `keyword.operator.math.cypher` },
        { match: `(<=|=>|<>|<|>|=~|=)`, name: `keyword.operator.compare.cypher` },
        { match: `(?i)\\b(OR|AND|XOR|IS)\\b`, name: `keyword.operator.logical.cypher` },
        { match: `(?i)\\b(IN)\\b`, name: `keyword.operator.in.cypher` },
      ],
    },
    'path-patterns': {
      patterns: [{ match: `(<--|-->|--)`, name: `support.function.relationship-pattern.cypher` }, {
        begin: `(<-|-)(\\[)`,
        beginCaptures: {
        ...
        endCaptures: {
          1: { name: `keyword.operator.relationship-pattern-end.cypher` },
          2: { name: `support.function.relationship-pattern-end.cypher` },
        },
    ...
    properties_literal: {
      patterns: [{
        begin: `{`,
        beginCaptures: { 0: { name: `keyword.control.properties_literal.cypher` } },
        end: `}`,
        endCaptures: { 0: { name: `keyword.control.properties_literal.cypher` } },
        name: `source.cypher`,
        patterns: [
          { match: `:|,`, name: `keyword.control.properties_literal.seperator.cypher` },
          { include: `#comments` },
          { include: `#constants` },
          { include: `#functions` },
          { include: `#operators` },
    ...
    strings: {
      ...
      }, { begin: `"`, end: `"`, name: `string.quoted.double.cypher`, patterns: [{ include: `#string_escape` }] }],
    },
  },
  a = `source.cypher`,
