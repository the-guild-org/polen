var e = `Blade`,
  ...
  a = `blade`,
  o = [{ include: `text.html.basic` }],
  s = {
    balance_brackets: {
      patterns: [{ begin: `\\(`, end: `\\)`, patterns: [{ include: `#balance_brackets` }] }, { match: `[^()]+` }],
    },
    blade: {
      patterns: [
        {
          begin: `{{--`,
          beginCaptures: { 0: { name: `punctuation.definition.comment.begin.blade` } },
          end: `--}}`,
          endCaptures: { 0: { name: `punctuation.definition.comment.end.blade` } },
        ...
        {
          begin: `(?<!@){{{`,
          beginCaptures: { 0: { name: `support.function.construct.begin.blade` } },
          contentName: `source.php`,
          end: `}}}`,
          endCaptures: { 0: { name: `support.function.construct.end.blade` }, 1: { name: `source.php` } },
        ...
        {
          begin: `(@){{`,
          beginCaptures: { 0: { name: `begin.bracket.round.blade` }, 1: { name: `variable.other.index.php` } },
          contentName: `source.php`,
          end: `}}`,
          endCaptures: { 0: { name: `end.bracket.round.blade` }, 1: { name: `source.php` } },
          name: `meta.function.echo.blade`,
          patterns: [{ include: `#language` }],
        },
        {
...
`,
          beginCaptures: { 1: { name: `keyword.blade` }, 2: { name: `begin.bracket.round.blade.php` } },
          contentName: `source.php`,
          end: `\\)`,
          endCaptures: { 0: { name: `end.bracket.round.blade.php` } },
...
`,
          ...
          endCaptures: { 0: { name: `end.bracket.round.blade.php` } },
          name: `meta.directive.blade`,
          patterns: [{ include: `#balance_brackets` }],
        },
        {
...
`,
        ...
        },
        {
          ...
          beginCaptures: {
            ...
            2: { name: `begin.bracket.round.blade.php` },
          },
          contentName: `comment.blade`,
          end: `\\)`,
          endCaptures: { 0: { name: `end.bracket.round.blade.php` } },
        ...
        { match: `(?x)(?<![A-Za-z0-9_@]) @(?:(?i)endphp|endsetup)\\b`, name: `punctuation.section.embedded.end.php` },
        {
          begin: `(?x)(?<![A-Za-z0-9_@]) (@\\w+(?:::w+)?[\\t ]*) (\\() # Followed by opening parentheses`,
          beginCaptures: { 1: { name: `entity.name.function.blade` }, 2: { name: `begin.bracket.round.blade.php` } },
          contentName: `source.php`,
          end: `\\)`,
          endCaptures: { 0: { name: `end.bracket.round.blade.php` } },
        ...
        { match: `(?x)(?<![A-Za-z0-9_@]) @\\w+(?:::w+)?\\b`, name: `entity.name.function.blade` },
      ],
    },
    'class-builtin': {
      patterns: [{
...
\\b`,
      ...
      }, { match: `(?i)[a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*`, name: `constant.other.php` }],
    },
    'function-call': {
      patterns: [{
        begin: `(?xi)
...
\\s*(=)\\s*(array)\\s*(\\()`,
        beginCaptures: {
          ...
          7: { name: `punctuation.definition.array.begin.bracket.round.php` },
        },
        ...
        patterns: [{ include: `#comments` }, { include: `#strings` }, { include: `#numbers` }],
      }, {
        captures: {
          1: { name: `storage.type.php` },
          2: { name: `variable.other.php` },
          ...
          7: { name: `punctuation.section.array.begin.php` },
          8: { patterns: [{ include: `#parameter-default-types` }] },
          9: { name: `punctuation.section.array.end.php` },
          10: { name: `invalid.illegal.non-null-typehinted.php` },
        },
        match: `(?xi)
(array|callable)
\\s+((&)?\\s*(\\$+)[a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)
...
\\s+((&)?\\s*(\\.\\.\\.)?(\\$+)[a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)`,
        beginCaptures: {
          1: {
            ...
            patterns: [{ match: `(?i)[a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*`, name: `storage.type.php` }, {
              match: `\\\\`,
          ...
          5: { name: `keyword.operator.variadic.php` },
          6: { name: `punctuation.definition.variable.php` },
        },
        end: `(?=,|\\)|/[/*]|\\#)`,
        ...
        captures: {
          ...
          3: { name: `keyword.operator.variadic.php` },
          4: { name: `punctuation.definition.variable.php` },
        },
        match: `(?xi)
...
(?:(\\[)((?>[^\\[\\]]+|\\[\\g<6>\\])*)(\\]))?`,
        beginCaptures: {
          ...
          7: { patterns: [{ include: `#parameter-default-types` }] },
    ...
    instantiation: {
      ...
      patterns: [{ match: `(?i)(parent|static|self)(?![a-z0-9_\\x{7f}-\\x{ff}])`, name: `storage.type.php` }, {
        include: `#class-name`,
    ...
    'invoke-call': {
      captures: { 1: { name: `punctuation.definition.variable.php` }, 2: { name: `variable.other.php` } },
      ...
      patterns: [
        { include: `#comments` },
        {
          ...
          beginCaptures: {
            ...
            `(?i)((?:[a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*\\s*,\\s*)*)([a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)?\\s*(?:(?={)|$)`,
          endCaptures: {
            1: {
              patterns: [{
                match: `(?i)[a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*`,
...
\\b`,
        ...
        },
        {
          ...
          beginCaptures: {
            1: { name: 'storage.modifier.${1:/downcase}.php' },
          ...
          patterns: [{ include: `#comments` }, {
            ...
            patterns: [
              ...
              { include: `#class-builtin` },
              { include: `#namespace` },
              { match: `(?i)[a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*`, name: `entity.other.inherited-class.php` },
            ],
          }, {
...
((\\$+)[a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)`,
        ...
        { match: `\\b(catch|try|throw|exception|finally)\\b`, name: `keyword.control.exception.php` },
        {
          ...
          patterns: [{
            begin: `\\(`,
            beginCaptures: { 0: { name: `punctuation.definition.parameters.begin.bracket.round.php` } },
...
\\s*(\\()`,
          beginCaptures: {
            1: {
              patterns: [{ match: `final|abstract|public|private|protected|static`, name: `storage.modifier.php` }],
            },
            2: { name: `storage.type.function.php` },
            3: { name: `support.function.magic.php` },
            4: { name: `entity.name.function.php` },
            5: { name: `punctuation.definition.parameters.begin.bracket.round.php` },
          },
          contentName: `meta.function.parameters.php`,
    ...
    namespace: {
      begin:
        `(?i)(?:(namespace)|[a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)?(\\\\)(?=.*?[^a-z0-9_\\x{7f}-\\x{ff}\\\\])`,
      beginCaptures: {
    ...
    numbers: {
      patterns: [
        { match: `0[xX][0-9a-fA-F]+`, name: `constant.numeric.hex.php` },
        { match: `0[bB][01]+`, name: `constant.numeric.binary.php` },
        { match: `0[0-7]+`, name: `constant.numeric.octal.php` },
        {
          captures: {
...
[0-9]+[eE][+-]?[0-9]+
        ...
        { match: `0|[1-9][0-9]*`, name: `constant.numeric.decimal.php` },
      ],
    },
    object: {
      patterns: [{
...
([a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)?
    ...
    php_doc: {
      patterns: [
        ...
        {
          begin:
            `(@(?:global|param|property(-(read|write))?|return|throws|var))\\s+(?=[A-Za-z_\\x{7f}-\\x{ff}\\\\]|\\()`,
...
|array|resource|void|null|callback|false|true|self)\\b`,
      ...
      match:
        `(?i)[a-z_\\x{7f}-\\x{ff}\\\\][a-z0-9_\\x{7f}-\\x{ff}\\\\]*(\\|[a-z_\\x{7f}-\\x{ff}\\\\][a-z0-9_\\x{7f}-\\x{ff}\\\\]*)*`,
    },
    ...
    'scope-resolution': {
      patterns: [{
        captures: {
          1: {
            patterns: [
              { match: `\\b(self|static|parent)\\b`, name: `storage.type.php` },
              { match: `\\w+`, name: `entity.name.class.php` },
              { include: `#class-name` },
              { include: `#variable-name` },
            ],
          },
...
([a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)
    ...
    'sql-string-double-quoted': {
      ...
      patterns: [
        ...
        { begin: `'`, end: `'`, name: `string.quoted.single.sql`, patterns: [{ include: `#interpolation` }] },
        { begin: '`', end: '`', name: `string.quoted.other.backtick.sql`, patterns: [{ include: `#interpolation` }] },
        { include: `#interpolation` },
        { include: `source.sql` },
      ],
    },
...
pow|eqv|fix|and|add|abs|round|get_type|xor|mod|mul)
        ...
        { begin: `(?i)\\b(isset|unset|eval|empty|list)\\b`, name: `support.function.construct.php` },
        { match: `(?i)\\b(print|echo)\\b`, name: `support.function.construct.output.php` },
        {
...
broker_(set_ordering|init|dict_exists|describe|free(_dict)?|list_dicts|request_(pwl_)?dict|get_error)
        ...
        { match: `(?i)\\bsplit(i)?|sql_regcase|ereg(i)?(_replace)?\\b`, name: `support.function.ereg.php` },
        {
          match:
