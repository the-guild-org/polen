var e = `Astro`,
  ...
  r = `astro`,
  i = [{ include: `#scope` }, { include: `#frontmatter` }, { include: `#text` }],
  a = {
    'attribute-literal': {
      begin: '(`)',
      end: `\\1`,
    ...
    attributes: {
      patterns: [{ include: `#attributes-events` }, { include: `#attributes-keyvalue` }, {
    ...
    'attributes-events': {
      ...
      beginCaptures: { 0: { patterns: [{ match: `.*`, name: `entity.other.attribute-name.astro` }] } },
      end: `(?=\\s*+[^=\\s])`,
      ...
      patterns: [{
        ...
        patterns: [{ include: `#interpolation` }, { include: `#attribute-literal` }, {
          begin: '(?=[^\\s=<>`/]|/(?!>))',
          end: `(?!\\G)`,
    ...
    'attributes-value': {
      patterns: [{ include: `#interpolation` }, { match: `([^\\s"'=<>\`/]|/(?!>))+`, name: `string.unquoted.astro` }, {
        begin: `(['"])`,
        beginCaptures: { 0: { name: `punctuation.definition.string.begin.astro` } },
        end: `\\1`,
        endCaptures: { 0: { name: `punctuation.definition.string.end.astro` } },
    ...
    entities: {
      patterns: [{
        captures: {
          1: { name: `punctuation.definition.entity.astro` },
          912: { name: `punctuation.definition.entity.astro` },
        },
        match: `(?x)
(&)	(?=[a-zA-Z])
(
...
`,
        ...
        captures: {
          1: { name: `punctuation.definition.entity.astro` },
          3: { name: `punctuation.definition.entity.astro` },
        },
        match: `(&)#[0-9]+(;)`,
      ...
      }, { match: `&(?=[a-zA-Z0-9]+;)`, name: `invalid.illegal.ambiguous-ampersand.astro` }],
    },
    frontmatter: {
      begin: `\\A(-{3})\\s*$`,
    ...
    interpolation: {
      patterns: [{
        ...
        patterns: [{ begin: `\\G\\s*(?={)`, end: `(?<=})`, patterns: [{ include: `source.tsx#object-literal` }] }, {
          include: `source.tsx`,
        }],
      }],
    },
    scope: {
      patterns: [{ include: `#comments` }, { include: `#tags` }, { include: `#interpolation` }, {
        include: `#entities`,
      }],
    },
    tags: {
      patterns: [{ include: `#tags-raw` }, { include: `#tags-lang` }, { include: `#tags-void` }, {
    ...
    'tags-general-end': {
      ...
      endCaptures: { 1: { name: `meta.tag.end.astro punctuation.definition.tag.end.astro` } },
    ...
    'tags-general-start': {
      ...
      endCaptures: { 1: { name: `meta.tag.start.astro punctuation.definition.tag.end.astro` } },
      name: `meta.scope.tag.$2.astro`,
    ...
    'tags-name': {
      patterns: [{ match: `[A-Z][a-zA-Z0-9_]*`, name: `support.class.component.astro` }, {
        match: `[a-z][\\w0-9:]*-[\\w0-9:-]*`,
    ...
    'tags-start-node': {
      captures: { 1: { name: `punctuation.definition.tag.begin.astro` }, 2: { patterns: [{ include: `#tags-name` }] } },
      match: `(<)([^/\\s>/]*)`,
      name: `meta.tag.start.astro`,
    },
    'tags-void': {
      begin: `(<)(area|base|br|col|embed|hr|img|input|link|meta|param|source|track|wbr)(?=\\s|/?>)`,
      beginCaptures: { 1: { name: `punctuation.definition.tag.begin.astro` }, 2: { name: `entity.name.tag.astro` } },
      end: `/?>`,
