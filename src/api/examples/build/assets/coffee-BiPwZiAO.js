var e = `CoffeeScript`,
  ...
  n = [
    ...
    },
    {
      ...
      patterns: [{
        captures: { 1: { name: `punctuation.definition.escape.backslash.coffee` } },
        match: `(\\\\).`,
    ...
    },
    {
      captures: {
        ...
        2: { name: `source.js.embedded.coffee`, patterns: [{ include: `source.js` }] },
    ...
    },
    {
      ...
      patterns: [{ match: `(?<=^|\\s)@\\w*(?=\\s)`, name: `storage.type.annotation.coffee` }],
    },
    {
      begin: `#`,
      beginCaptures: { 0: { name: `punctuation.definition.comment.coffee` } },
    ...
    { match: `\\b(?<![\\.\\$])(delete|instanceof|new|typeof)(?!\\s*:)\\b`, name: `keyword.operator.$1.coffee` },
    {
      match:
...
(?=(\\([^\\(\\)]*\\)\\s*)?[=-]>)`,
    ...
    {
      begin: `(?<=\\s|^)({)(?=[^'"#]+?}[\\s\\]}]*=)`,
      beginCaptures: { 1: { name: `punctuation.definition.destructuring.begin.bracket.curly.coffee` } },
      end: `}`,
      endCaptures: { 0: { name: `punctuation.definition.destructuring.end.bracket.curly.coffee` } },
      name: `meta.variable.assignment.destructured.object.coffee`,
      patterns: [{ include: `$self` }, { match: `[a-zA-Z$_]\\w*`, name: `variable.assignment.coffee` }],
    },
    {
      begin: `(?<=\\s|^)(\\[)(?=[^'"#]+?\\][\\s\\]}]*=)`,
      beginCaptures: { 1: { name: `punctuation.definition.destructuring.begin.bracket.square.coffee` } },
      end: `\\]`,
      endCaptures: { 0: { name: `punctuation.definition.destructuring.end.bracket.square.coffee` } },
      name: `meta.variable.assignment.destructured.array.coffee`,
      patterns: [{ include: `$self` }, { match: `[a-zA-Z$_]\\w*`, name: `variable.assignment.coffee` }],
    },
    { match: `\\b(?<!\\.|::)(true|on|yes)(?!\\s*[:=][^=])\\b`, name: `constant.language.boolean.true.coffee` },
    { match: `\\b(?<!\\.|::)(false|off|no)(?!\\s*[:=][^=])\\b`, name: `constant.language.boolean.false.coffee` },
    { match: `\\b(?<!\\.|::)null(?!\\s*[:=][^=])\\b`, name: `constant.language.null.coffee` },
    { match: `\\b(?<!\\.|::)extends(?!\\s*[:=])\\b`, name: `variable.language.coffee` },
    { match: `(?<!\\.)\\b(?<!\\$)(super|this|arguments)(?!\\s*[:=][^=]|\\$)\\b`, name: `variable.language.$1.coffee` },
    {
      captures: {
    ...
    { match: `((?<=Number\\.)(is(Finite|Integer|NaN)|toInteger))\\b`, name: `support.function.static.number.coffee` },
    {
    ...
    { match: `(?<!\\$)\\b[0-9]+[\\w$]*`, name: `invalid.illegal.identifier.coffee` },
    ...
    {
      begin: `\\[`,
      beginCaptures: { 0: { name: `punctuation.definition.array.begin.bracket.square.coffee` } },
      end: `\\]`,
      endCaptures: { 0: { name: `punctuation.definition.array.end.bracket.square.coffee` } },
      patterns: [{ match: `(?<!\\.)\\.{3}`, name: `keyword.operator.slice.exclusive.coffee` }, {
        match: `(?<!\\.)\\.{2}`,
  ...
  r = {
    arguments: {
      patterns: [{
        begin: `\\(`,
        beginCaptures: { 0: { name: `punctuation.definition.arguments.begin.bracket.round.coffee` } },
        end: `\\)`,
        ...
        begin: `(?=(@|@?[\\w$]+|[=-]>|\\-\\d|\\[|{|"|'))`,
          ...
          `(?=\\s*(?<![\\w$])(of|in|then|is|isnt|and|or|for|else|when|if|unless|by|instanceof)(?![\\w$]))|(?=\\s*(}|\\]|\\)|#|$))`,
        name: `meta.arguments.coffee`,
        patterns: [{ include: `$self` }],
      }],
    },
...
require|set(Interval|Timeout)|clear(Interval|Timeout))\\b`,
    ...
    function_params: {
      patterns: [{
        begin: `\\(`,
        beginCaptures: { 0: { name: `punctuation.definition.parameters.begin.bracket.round.coffee` } },
        end: `\\)`,
        endCaptures: { 0: { name: `punctuation.definition.parameters.end.bracket.round.coffee` } },
        name: `meta.parameters.coffee`,
        patterns: [{
          captures: { 1: { name: `variable.parameter.function.coffee` }, 2: { name: `keyword.operator.splat.coffee` } },
          match: `([a-zA-Z_$][\\w$]*)(\\.\\.\\.)?`,
        }, {
          captures: {
            1: { name: `variable.parameter.function.readwrite.instance.coffee` },
    ...
    instance_variable: {
      patterns: [{ match: `(@)([a-zA-Z_\\$]\\w*)?`, name: `variable.other.readwrite.instance.coffee` }],
    },
    interpolated_coffee: {
      patterns: [{
        begin: `\\#\\{`,
    ...
    'jsx-end-tag': {
      patterns: [{
        begin: `(</)([-\\w\\.]+)`,
        beginCaptures: { 1: { name: `punctuation.definition.tag.coffee` }, 2: { name: `entity.name.tag.coffee` } },
        end: `(/?>)`,
...
toDataURL|toggle|toString|values|write|writeln)\\b`,
    ...
    numbers: {
      patterns: [
        { match: `\\b(?<!\\$)0(x|X)[0-9a-fA-F]+\\b(?!\\$)`, name: `constant.numeric.hex.coffee` },
        { match: `\\b(?<!\\$)0(b|B)[01]+\\b(?!\\$)`, name: `constant.numeric.binary.coffee` },
        { match: `\\b(?<!\\$)0(o|O)?[0-7]+\\b(?!\\$)`, name: `constant.numeric.octal.coffee` },
        {
          captures: {
...
(?:\\b[0-9]+\\b(?!\\.))
    ...
    objects: {
      ...
      }, { match: `[a-zA-Z_$][\\w$]*(?=\\s*\\??(\\.\\s*[a-zA-Z_$]\\w*|::))`, name: `variable.other.object.coffee` }],
    },
    operators: {
      patterns: [
        {
        ...
        {
          captures: { 1: { name: `variable.assignment.coffee` }, 2: { name: `keyword.operator.assignment.coffee` } },
          match: `([a-zA-Z$_][\\w$]*)?\\s*(=|:(?!:))(?![>=])`,
        },
