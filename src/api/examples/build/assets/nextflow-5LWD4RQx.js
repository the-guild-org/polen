var e = `Nextflow`,
  ...
  r = {
    ...
    'include-statement': {
      patterns: [{ match: `^\\b(include)\\b`, name: `keyword.nextflow` }, {
        match: `\\b(from)\\b`,
    ...
    'nfl-rules': {
      patterns: [
        ...
        {
          begin: `(\\w*\\()`,
          beginCaptures: { 1: { patterns: [{ include: `source.nextflow-groovy` }] } },
          comment: `method call and parens`,
          end: `\\)`,
          patterns: [{ include: `#nfl-rules` }],
        },
        { begin: `{`, comment: `braces`, end: `}`, patterns: [{ include: `#nfl-rules` }] },
    ...
    'process-body': {
      patterns: [
        {
          match:
            `(?:accelerator|afterScript|beforeScript|cache|cpus|conda|container|containerOptions|clusterOptions|debug|disk|echo|errorStrategy|executor|ext|label|machineType|maxErrors|maxForks|maxRetries|memory|module|penv|pod|publishDir|queue|resourceLabels|scratch|stageInMode|stageOutMode|storeDir|tag|time)\\b`,
          name: `entity.name.function.nextflow`,
        },
        { match: `(?:input|output|when|script|shell|exec):`, name: `constant.block.nextflow` },
        { match: `\\b(tuple|set|path|file|val|stdout)(\\(|\\s)`, name: `entity.name.function.nextflow` },
        { include: `#implicit-variables` },
        {
          begin: `(\\w*\\()`,
          beginCaptures: { 1: { patterns: [{ include: `source.nextflow-groovy` }] } },
    ...
    'workflow-body': {
      patterns: [
        { include: `#implicit-variables` },
        { match: `(?:take|main|emit):`, name: `constant.block.nextflow` },
        {
          match:
            `(?<=[\\s\\.])(branch|buffer|close|collate|collect|collectFile|combine|concat|count|countBy|cross|distinct|dump|filter|first|flatMap|flatten|groupTuple|ifEmpty|join|last|map|max|merge|min|mix|multiMap|randomSample|reduce|set|splitCsv|splitFasta|splitFastq|splitText|sum|take|tap|toInteger|toList|toSortedList|transpose|unique|until|view)(?=[{\\(\\s])`,
          name: `entity.name.function.nextflow`,
        },
        {
          captures: { 1: { name: `keyword.nextflow` }, 2: { name: `entity.name.function.nextflow` } },
        ...
        },
        {
          ...
          beginCaptures: { 1: { name: `keyword.nextflow` } },
        ...
        { begin: `{`, comment: `braces`, end: `}`, patterns: [{ include: `#workflow-body` }] },
        { include: `source.nextflow-groovy#comments` },
        { include: `source.nextflow-groovy#support-functions` },
        { include: `source.nextflow-groovy#keyword` },
        { include: `source.nextflow-groovy#values` },
    ...
    'workflow-def': {
      ...
      beginCaptures: { 1: { name: `keyword.nextflow` }, 2: { name: `constant.nextflow` } },
      end: `}`,
