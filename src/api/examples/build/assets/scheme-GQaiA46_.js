var e = `Scheme`,
  ...
  i = {
    ...
    comment: {
      begin: `(^[ \\t]+)?(?=;)`,
      beginCaptures: { 1: { name: `punctuation.whitespace.comment.leading.scheme` } },
      end: `(?!\\G)`,
      patterns: [{
        begin: `;`,
    ...
    constants: {
      patterns: [{ match: `#[t|f]`, name: `constant.language.boolean.scheme` }, {
...
`,
          ...
          patterns: [{ include: `#comment` }, { include: `#sexp` }, { include: `#illegal` }],
        },
        {
          begin: `(?x)
(?<=\\()
...
`,
        ...
        },
        {
          ...
          captures: { 1: { name: `keyword.control.scheme` }, 2: { name: `variable.other.scheme` } },
          end: `(?=\\))`,
        ...
        { include: `#string` },
        { include: `#constants` },
        { match: `(?<=[\\(\\s])(#\\\\)(space|newline|tab)(?=[\\s\\)])`, name: `constant.character.named.scheme` },
        { match: `(?<=[\\(\\s])(#\\\\)x[0-9A-F]{2,4}(?=[\\s\\)])`, name: `constant.character.hex-literal.scheme` },
        { match: `(?<=[\\(\\s])(#\\\\).(?=[\\s\\)])`, name: `constant.character.escape.scheme` },
        {
          comment: `
