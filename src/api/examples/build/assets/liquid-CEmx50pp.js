...
`,
  ...
  s = {
    ...
    comment_block: {
      ...
      patterns: [{ include: `#comment_block` }, { match: `(.(?!{%-?\\s*(comment|endcomment)\\s*-?%}))*.` }],
    },
    core: {
      patterns: [
        { include: `#raw_tag` },
    ...
    injection: {
      patterns: [{ include: `#raw_tag` }, { include: `#comment_block` }, { include: `#object` }, {
        include: `#tag_injection`,
      }],
    },
    invalid_range: { match: `\\((.(?!\\.\\.))+\\)`, name: `invalid.illegal.range.liquid` },
    ...
    language_constant: { match: `\\b(false|true|nil|blank)\\b|empty(?!\\?)`, name: `constant.language.liquid` },
    number: { match: `((-|\\+)\\s*)?[0-9]+(\\.[0-9]+)?`, name: `constant.numeric.liquid` },
    object: {
      begin:
      ...
      patterns: [{ include: `#filter` }, { include: `#attribute` }, { include: `#value_expression` }],
    },
    operator: {
    ...
    range: {
      ...
      patterns: [{ match: `\\.\\.`, name: `punctuation.range.liquid` }, { include: `#variable_lookup` }, {
        include: `#number`,
      }],
    },
    raw_tag: {
    ...
    string: { patterns: [{ include: `#string_single` }, { include: `#string_double` }] },
    ...
    tag_assign_liquid: {
      ...
      name: `meta.entity.tag.liquid`,
      patterns: [{ include: `#filter` }, { include: `#attribute_liquid` }, { include: `#value_expression` }],
    },
    tag_body: {
      patterns: [
        { include: `#tag_liquid` },
    ...
    tag_comment_block_liquid: {
      ...
      name: `comment.block.liquid`,
      patterns: [{ include: `#tag_comment_block_liquid` }, { match: `(?:^\\s*)(?!(comment|endcomment)).*` }],
    },
    tag_comment_inline: { begin: `#`, end: `(?=%})`, name: `comment.line.number-sign.liquid` },
    tag_comment_inline_liquid: { begin: `(?:^\\s*)#.*`, end: `$`, name: `comment.line.number-sign.liquid` },
    tag_conditional: {
    ...
    tag_expression: {
      patterns: [{ include: `#tag_expression_without_arguments` }, {
        begin: `(?:(?:(?<={%)|(?<={%-)|^)\\s*)(\\w+)`,
        beginCaptures: { 1: { name: `entity.name.tag.liquid` } },
        end: `(?=%})`,
        name: `meta.entity.tag.liquid`,
    ...
    tag_for_body: {
      patterns: [{ match: `\\b(in|reversed)\\b`, name: `keyword.control.liquid` }, {
    ...
    tag_paginate_body: {
      patterns: [{ match: `\\b(by)\\b`, name: `keyword.control.liquid` }, { include: `#value_expression` }],
    },
    tag_paginate_liquid: {
      begin: `(?:(?:(?<={%)|(?<={%-)|^)\\s*)(paginate)\\b`,
      beginCaptures: { 1: { name: `keyword.control.paginate.liquid` } },
    ...
    tag_render: {
      ...
      patterns: [{ include: `#tag_render_special_keywords` }, { include: `#attribute` }, {
        include: `#value_expression`,
      }],
    },
    tag_render_liquid: {
      ...
      name: `meta.entity.tag.render.liquid`,
      patterns: [{ include: `#tag_render_special_keywords` }, { include: `#attribute_liquid` }, {
        include: `#value_expression`,
      }],
    },
    tag_render_special_keywords: { match: `\\b(with|as|for)\\b`, name: `keyword.control.other.liquid` },
    tag_tablerow: {
      begin: `(?:(?:(?<={%)|(?<={%-)|^)\\s*)(tablerow)\\b`,
    ...
    tag_tablerow_body: {
      patterns: [{ match: `\\b(in)\\b`, name: `keyword.control.liquid` }, {
    ...
    value_expression: {
      patterns: [
        {
          captures: { 2: { name: `invalid.illegal.filter.liquid` }, 3: { name: `invalid.illegal.filter.liquid` } },
          match: `(\\[)(\\|)(?=[^\\]]*)(?=\\])`,
        },
    ...
    variable_lookup: {
      patterns: [
        ...
        {
          begin: `(?<=\\w)\\[`,
          beginCaptures: { 0: { name: `punctuation.section.brackets.begin.liquid` } },
          end: `\\]`,
          endCaptures: { 0: { name: `punctuation.section.brackets.end.liquid` } },
          name: `meta.brackets.liquid`,
          patterns: [{ include: `#string` }],
        },
        { match: `(?<=(\\w|\\])\\.)([-\\w]+\\??)`, name: `variable.other.member.liquid` },
        { match: `(?<=\\w)\\.(?=\\w)`, name: `punctuation.accessor.liquid` },
        { match: `(?i)[a-z_](\\w|(?:-(?!\\}\\})))*`, name: `variable.other.liquid` },
      ],
    },
  },
  c = `text.html.liquid`,
