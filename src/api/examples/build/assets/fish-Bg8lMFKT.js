var e = `Fish`,
  t = [`fish`],
  n = `^#!.*\\bfish\\b`,
  r = `^\\s*(function|while|if|switch|for|begin)\\s.*$`,
  i = `^\\s*end\\s*$`,
  a = `fish`,
  o = [
    {
    ...
    {
      captures: { 2: { name: `keyword.control.fish` }, 4: { name: `support.function.command.fish` } },
      comment: `name of command, either a function or a binary`,
      match:
        `(^\\s*|&&\\s*|\\|\\s*|\\(\\s*|[;]\\s*|\\b(if|while)\\b\\s+)(?!(?<!\\.)\\b(function|while|if|else|switch|case|for|in|begin|end|continue|break|return|source|exit|wait|and|or|not)\\b(?![?!]))([a-zA-Z_\\-0-9\\[\\].]+)`,
    },
    {
  ...
  s = {
    escape: {
      patterns: [{
        ...
        comment:
          `escapes a byte of data with the specified value (hexadecimal). If you are using mutibyte encoding, this can be used to enter invalid strings. Only use this if you know what are doing.`,
        match: `\\\\X[0-9a-fA-F]{1,2}`,
