var e = `BibTeX`,
  t = `bibtex`,
  n = [{
    captures: { 0: { name: `punctuation.definition.comment.bibtex` } },
    match: `@(?i:comment)(?=[\\s{(])`,
    name: `comment.block.at-sign.bibtex`,
  }, {
    begin: `((@)(?i:preamble))\\s*(\\{)\\s*`,
    beginCaptures: {
      1: { name: `keyword.other.preamble.bibtex` },
      2: { name: `punctuation.definition.keyword.bibtex` },
      3: { name: `punctuation.section.preamble.begin.bibtex` },
    },
    end: `\\}`,
    endCaptures: { 0: { name: `punctuation.section.preamble.end.bibtex` } },
    name: `meta.preamble.braces.bibtex`,
    patterns: [{ include: `#field_value` }],
  }, {
    begin: `((@)(?i:preamble))\\s*(\\()\\s*`,
    beginCaptures: {
      1: { name: `keyword.other.preamble.bibtex` },
    ...
    begin:
      '((@)(?i:string))\\s*(\\{)\\s*([a-zA-Z!$&*+\\-./:;<>?@\\[\\\\\\]^_`|~][a-zA-Z0-9!$&*+\\-./:;<>?@\\[\\\\\\]^_`|~]*)',
    beginCaptures: {
    ...
    patterns: [{
      ...
      beginCaptures: {
        1: { name: `support.function.key.bibtex` },
  ...
  r = {
    field_value: {
      patterns: [{ include: `#string_content` }, { include: `#integer` }, { include: `#string_var` }, {
        match: `#`,
        name: `keyword.operator.bibtex`,
      }],
    },
    integer: { captures: { 1: { name: `constant.numeric.bibtex` } }, match: `\\s*(\\d+)\\s*` },
    nested_braces: {
      begin: `\\{`,
    ...
    string_var: {
      captures: { 0: { name: `support.variable.bibtex` } },
