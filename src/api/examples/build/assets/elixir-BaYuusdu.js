var e = `Elixir`,
  t = [`ex`, `exs`],
  n = `^#!/.*\\belixir`,
  r = `(after|else|catch|rescue|\\-\\>|\\{|\\[|do)\\s*$`,
  i = `^\\s*((\\}|\\]|after|else|catch|rescue)\\s*$|end\\b)`,
  a = `elixir`,
  o = [
    {
      begin: `\\b(fn)\\b(?!.*->)`,
    ...
    {
      captures: { 1: { name: `keyword.operator.other.elixir` }, 2: { name: `entity.name.function.elixir` } },
      match: `(\\|\\>)\\s*([a-z_]\\w*[!?]?)`,
    },
    { match: `\\b[a-z_]\\w*[!?]?(?=\\s*\\.?\\s*\\()`, name: `entity.name.function.elixir` },
    {
      begin: `\\b(fn)\\b(?=.*->)`,
      beginCaptures: { 1: { name: `keyword.control.elixir` } },
      end: `(?>(->)|(when)|(\\)))`,
  ...
  s = {
      ...
      patterns: [
        {
          ...
          patterns: [{ match: `\\b[A-Z]\\w*(?=\\.)`, name: `entity.other.inherited-class.elixir` }, {
            match: `\\b[A-Z]\\w*\\b`,
        ...
        },
        {
          ...
          patterns: [{ match: `\\b[A-Z]\\w*\\b`, name: `entity.name.type.protocol.elixir` }],
        },
        {
          begin: `^\\s*(defimpl)\\b`,
          beginCaptures: { 1: { name: `keyword.control.protocol.elixir` } },
        ...
        },
        {
          ...
          endCaptures: { 1: { name: `constant.other.keywords.elixir` }, 2: { name: `keyword.control.module.elixir` } },
        ...
        },
        {
          ...
          patterns: [{ include: `#interpolated_elixir` }, { include: `#escaped_char` }],
        },
        {
          begin: `@(module|type)?doc ~[A-Z]"""`,
          comment: `@doc with heredocs is treated as documentation`,
        ...
        { match: `\\b(nil|true|false)\\b(?![?!])`, name: `constant.language.elixir` },
        { match: `\\b(__(CALLER|ENV|MODULE|DIR|STACKTRACE)__)\\b(?![?!])`, name: `variable.language.elixir` },
        {
        ...
        { match: `\\b0x[0-9A-Fa-f](?>_?[0-9A-Fa-f])*\\b`, name: `constant.numeric.hex.elixir` },
        {
        ...
        { match: `\\b0b[01](?>_?[01])*\\b`, name: `constant.numeric.binary.elixir` },
        { match: `\\b0o[0-7](?>_?[0-7])*\\b`, name: `constant.numeric.octal.elixir` },
        {
          begin: `:'`,
        ...
        {
          begin: `(^[ \\t]+)?(?=##)`,
          beginCaptures: { 1: { name: `punctuation.whitespace.comment.leading.elixir` } },
          end: `(?!#)`,
          patterns: [{
            begin: `##`,
        ...
        { match: `\\|\\>|<~>|<>|<<<|>>>|~>>|<<~|~>|<~|<\\|>`, name: `keyword.operator.sigils_1.elixir` },
        { match: `&&&|&&`, name: `keyword.operator.sigils_2.elixir` },
        { match: `<\\-|\\\\\\\\`, name: `keyword.operator.sigils_3.elixir` },
        { match: `===?|!==?|<=?|>=?`, name: `keyword.operator.comparison.elixir` },
        { match: `(\\|\\|\\||&&&|\\^\\^\\^|<<<|>>>|~~~)`, name: `keyword.operator.bitwise.elixir` },
        {
    ...
    escaped_char: { match: `\\\\(x[\\da-fA-F]{1,2}|.)`, name: `constant.character.escaped.elixir` },
    interpolated_elixir: {
      begin: `#\\{`,
