var e = `Smalltalk`,
  ...
  a = [
    ...
    },
    {
      captures: {
        ...
        2: { patterns: [{ match: `\\w+`, name: `support.type.variable.declaration.smalltalk` }] },
        3: { name: `punctuation.definition.instance-variables.end.smalltalk` },
      },
      match: `(\\|)(\\s*\\w[\\w ]*)(\\|)`,
    },
    {
      captures: { 1: { patterns: [{ match: `:\\w+`, name: `entity.name.function.block.smalltalk` }] } },
      comment: `Parse the blocks like: [ :a :b | ...... ]`,
      match: `\\[((\\s+|:\\w+)*)\\|`,
    },
    { include: `#numeric` },
    { match: `<(?!<|=)|>(?!<|=|>)|<=|>=|=|==|~=|~~|>>|\\^`, name: `keyword.operator.comparison.smalltalk` },
    { match: `(\\*|\\+|\\-|/|\\\\)`, name: `keyword.operator.arithmetic.smalltalk` },
    { match: `(?<=[ \\t])!+|\\bnot\\b|&|\\band\\b|\\||\\bor\\b`, name: `keyword.operator.logical.smalltalk` },
    {
    ...
    {
      begin: `"`,
      beginCaptures: [{ name: `punctuation.definition.comment.begin.smalltalk` }],
      end: `"`,
      endCaptures: [{ name: `punctuation.definition.comment.end.smalltalk` }],
    ...
    {
      begin: `#\\[`,
      beginCaptures: [{ name: `punctuation.definition.constant.begin.smalltalk` }],
      end: `\\]`,
      endCaptures: [{ name: `punctuation.definition.constant.end.smalltalk` }],
      name: `meta.array.byte.smalltalk`,
      patterns: [{ match: `[0-9]+(r[a-zA-Z0-9]+)?`, name: `constant.numeric.integer.smalltalk` }, {
        match: `[^\\s\\]]+`,
    ...
    {
      begin: `'`,
      beginCaptures: [{ name: `punctuation.definition.string.begin.smalltalk` }],
      end: `'`,
      endCaptures: [{ name: `punctuation.definition.string.end.smalltalk` }],
  ...
  o = {
    numeric: {
      patterns: [
        { match: `(?<!\\w)[0-9]+\\.[0-9]+s[0-9]*`, name: `constant.numeric.float.scaled.smalltalk` },
        { match: `(?<!\\w)[0-9]+\\.[0-9]+([edq]-?[0-9]+)?`, name: `constant.numeric.float.smalltalk` },
        { match: `(?<!\\w)-?[0-9]+r[a-zA-Z0-9]+`, name: `constant.numeric.integer.radix.smalltalk` },
        { match: `(?<!\\w)-?[0-9]+([edq]-?[0-9]+)?`, name: `constant.numeric.integer.smalltalk` },
      ],
    },
  },
  s = `source.smalltalk`,
