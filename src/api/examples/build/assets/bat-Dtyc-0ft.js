var e = `Batch File`,
  t = { 'L:meta.block.repeat.batchfile': { patterns: [{ include: `#repeatParameter` }] } },
  n = `bat`,
  r = [
    { include: `#commands` },
    { include: `#comments` },
  ...
  i = {
    ...
    command_set_group: {
      patterns: [{
        begin: `\\(`,
        beginCaptures: { 0: { name: `punctuation.section.group.begin.batchfile` } },
        end: `\\)`,
    ...
    command_set_inside: {
      patterns: [
        ...
        { include: `#strings` },
        {
          ...
          patterns: [{ include: `#escaped_characters` }, { include: `#variables` }, { include: `#numbers` }, {
            include: `#parens`,
          }, { include: `#strings` }],
        },
        {
          ...
          patterns: [
            {
              begin: `"`,
              beginCaptures: { 0: { name: `punctuation.definition.string.begin.batchfile` } },
              end: `"`,
              endCaptures: { 0: { name: `punctuation.definition.string.end.batchfile` } },
              name: `string.quoted.double.batchfile`,
              patterns: [{ include: `#command_set_inside_arithmetic` }, { include: `#command_set_group` }, {
                include: `#variables`,
              }],
            },
        ...
        },
        {
          ...
          patterns: [{ include: `#command_set_strings` }, {
            begin: `([^ ][^=]*)(=)`,
            beginCaptures: {
              1: { name: `variable.other.readwrite.batchfile` },
              2: { name: `keyword.operator.assignment.batchfile` },
    ...
    command_set_inside_arithmetic: {
      patterns: [{ include: `#command_set_operators` }, { include: `#numbers` }, {
        match: `,`,
    ...
    command_set_operators: {
      patterns: [
        ...
        { match: `\\+|\\-|/|\\*|%%|\\||&|\\^|<<|>>|~`, name: `keyword.operator.arithmetic.batchfile` },
    ...
    command_set_strings: {
      patterns: [{
        ...
        endCaptures: { 0: { name: `punctuation.definition.string.end.batchfile` } },
        name: `string.quoted.double.batchfile`,
        patterns: [{ include: `#variables` }, { include: `#numbers` }, { include: `#escaped_characters` }],
      }],
    },
    commands: {
      patterns: [{
        ...
        captures: { 1: { name: `keyword.command.batchfile` }, 2: { name: `keyword.other.special-method.batchfile` } },
        match:
    ...
    comments: {
      patterns: [{
        ...
        patterns: [{
          begin: `((?::[+=,;: ]))`,
          beginCaptures: { 1: { name: `punctuation.definition.comment.batchfile` } },
          end: `(?=\\n)`,
          name: `comment.line.colon.batchfile`,
        }],
      }, {
        begin: `(?<=^|[\\s@])(?i)(REM)(\\.)`,
        beginCaptures: { 1: { name: `keyword.command.rem.batchfile` }, 2: { name: `punctuation.separator.batchfile` } },
        end: `(?=$\\n|[&|><)])`,
        ...
        name: `comment.line.rem.batchfile`,
        patterns: [{ match: `[><|]`, name: `invalid.illegal.unexpected-character.batchfile` }],
      }],
    },
    constants: { patterns: [{ match: `\\b(?i:NUL)\\b`, name: `constant.language.batchfile` }] },
    controls: {
      patterns: [
        { match: `(?i)(?<=^|\\s)(?:call|exit(?=$|\\s)|goto(?=$|\\s|:))`, name: `keyword.control.statement.batchfile` },
        {
          captures: {
        ...
        { match: `(?<=^|\\s)(?i)(?:if|else)(?=$|\\s)`, name: `keyword.control.conditional.batchfile` },
        {
          begin: `(?<=^|[\\s(&^])(?i)for(?=\\s)`,
    ...
    operators: {
      patterns: [
        ...
        { match: `(?<=\\s)(?i:EQU|NEQ|LSS|LEQ|GTR|GEQ)(?=\\s)|==`, name: `keyword.operator.comparison.batchfile` },
        { match: `(?<=\\s)(?i)(NOT)(?=\\s)`, name: `keyword.operator.logical.batchfile` },
        { match: `(?<!\\^)&&?|\\|\\|`, name: `keyword.operator.conditional.batchfile` },
    ...
    parens: {
      patterns: [{
        ...
        patterns: [{ match: `,|;`, name: `punctuation.separator.batchfile` }, { include: `$self` }],
      }],
    },
    repeatParameter: {
      patterns: [{
    ...
    strings: {
      patterns: [{
        ...
        name: `string.quoted.double.batchfile`,
        patterns: [{ match: `%%`, name: `constant.character.escape.batchfile` }, { include: `#variables` }],
      }],
    },
    variable: {
      patterns: [{
        begin: `%(?=[^%]+%)`,
        beginCaptures: { 0: { name: `punctuation.definition.variable.begin.batchfile` } },
        end: `(%)|\\n`,
        endCaptures: { 1: { name: `punctuation.definition.variable.end.batchfile` } },
        ...
        patterns: [{
          ...
          patterns: [{ include: `#variable_replace` }, {
            begin: `=`,
            beginCaptures: { 0: { name: `punctuation.separator.batchfile` } },
            end: `(?=%|\\n)`,
            patterns: [{ include: `#variable_delayed_expansion` }, {
              match: `[^%]+`,
    ...
    variable_delayed_expansion: {
      patterns: [{
        ...
        patterns: [{
          ...
          name: `meta.variable.substitution.batchfile`,
          patterns: [{ include: `#escaped_characters` }, { include: `#variable_replace` }, { include: `#variable` }, {
            begin: `=`,
            beginCaptures: { 0: { name: `punctuation.separator.batchfile` } },
            end: `(?=!|\\n)`,
            patterns: [{ include: `#variable` }, { match: `[^!]+`, name: `string.unquoted.batchfile` }],
          }],
        }],
      }],
    },
    variable_replace: { patterns: [{ match: `[^=%!\\n]+`, name: `string.unquoted.batchfile` }] },
    variable_substring: {
      patterns: [{
        captures: {
  ...
  a = `source.batchfile`,
  o = { displayName: e, injections: t, name: `bat`, patterns: r, repository: i, scopeName: a }
