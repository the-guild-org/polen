var e = `Systemd Units`,
  ...
  n = [{ include: `#comments` }, {
    begin:
      `^\\s*(InaccessableDirectories|InaccessibleDirectories|ReadOnlyDirectories|ReadWriteDirectories|Capabilities|TableId|UseDomainName|IPv6AcceptRouterAdvertisements|SysVStartPriority|StartLimitInterval|RequiresOverridable|RequisiteOverridable|PropagateReloadTo|PropagateReloadFrom|OnFailureIsolate|BindTo)\\s*(=)[ \\t]*`,
    beginCaptures: { 1: { name: `invalid.deprecated` }, 2: { name: `keyword.operator.assignment` } },
    end: `(?<!\\\\)\\n`,
    patterns: [
      { include: `#comments` },
      { include: `#variables` },
    ...
    beginCaptures: { 1: { name: `entity.name.tag` }, 2: { name: `keyword.operator.assignment` } },
    end: `(?<!\\\\)\\n`,
    name: `meta.config-entry.systemd`,
    patterns: [
      { include: `#comments` },
      {
        captures: { 1: { name: `variable.parameter` }, 2: { name: `keyword.operator.assignment` } },
    ...
    name: `meta.config-entry.systemd`,
    patterns: [{ include: `#comments` }, { include: `#variables` }, { include: `#calendarShorthands` }, {
      include: `#numbers`,
    }],
  }, {
    ...
    name: `meta.config-entry.systemd`,
    patterns: [{ include: `#comments` }, { include: `#capabilities` }],
  }, {
    begin: `^\\s*(Restart)\\s*(=)[ \\t]*`,
    beginCaptures: { 1: { name: `entity.name.tag` }, 2: { name: `keyword.operator.assignment` } },
    end: `(?<!\\\\)\\n`,
    name: `meta.config-entry.systemd`,
    patterns: [{ include: `#comments` }, { include: `#variables` }, { include: `#restartOptions` }],
  }, {
    begin: `^\\s*(Type)\\s*(=)[ \\t]*`,
    beginCaptures: { 1: { name: `entity.name.tag` }, 2: { name: `keyword.operator.assignment` } },
    end: `(?<!\\\\)\\n`,
    name: `meta.config-entry.systemd`,
    patterns: [{ include: `#comments` }, { include: `#variables` }, { include: `#typeOptions` }],
  }, {
  ...
  r = {
    ...
    numbers: { patterns: [{ match: `(?<=\\s|=)\\d+(?:\\.\\d+)?(?=[\\s:]|$)`, name: `constant.numeric` }] },
    quotedString: {
      patterns: [{
        begin: `(?<=\\G|\\s)'`,
        end: `['\\n]`,
    ...
    timeSpans: {
      patterns: [{
        match:
          `\\b(?:\\d+(?:[uμ]s(?:ec)?|ms(?:ec)?|s(?:ec|econds?)?|m(?:in|inutes?)?|h(?:r|ours?)?|d(?:ays?)?|w(?:eeks)?|M|months?|y(?:ears?)?)){1,}\\b`,
        name: `constant.numeric`,
      }],
    },
    typeOptions: {
      patterns: [{
        match:
          `\\b(?:simple|exec|forking|oneshot|dbus|notify(?:-reload)?|idle|unicast|local|broadcast|anycast|multicast|blackhole|unreachable|prohibit|throw|nat|xresolve|blackhole|unreachable|prohibit|ad-hoc|station|ap(?:-vlan)?|wds|monitor|mesh-point|p2p-(?:client|go|device)|ocb|nan)\\b`,
        name: `constant.language`,
      }],
    },
    variables: {
      patterns: [
        {
          captures: { 1: { name: `punctuation.definition.variable.systemd` }, 2: { name: `variable.other` } },
          match: `(\\$)([A-Za-z0-9\\_]+)\\b`,
        },
        {
          captures: {
            1: { name: `punctuation.definition.variable.systemd` },
            2: { name: `variable.other` },
            3: { name: `punctuation.definition.variable.systemd` },
          },
          match: `(\\$\\{)([A-Za-z0-9\\_]+)(\\})`,
        },
        { match: `%%`, name: `constant.other.placeholder` },
        { match: `%[aAbBCEfgGhHiIjJlLmMnNopPsStTuUvVwW]\\b`, name: `constant.other.placeholder` },
      ],
    },
  },
  i = `source.systemd`,
