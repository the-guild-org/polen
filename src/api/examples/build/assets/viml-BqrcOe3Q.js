var e = `Vim Script`,
  ...
  r = {
    commands: {
      patterns: [
        { match: `\\bcom(\\s|\\!)`, name: `storage.other.command.viml` },
        { match: `\\bau(\\s|\\!)`, name: `storage.other.command.viml` },
        { match: `-bang`, name: `storage.other.command.bang.viml` },
        { match: `-nargs=[*+0-9]+`, name: `storage.other.command.args.viml` },
        { match: `-complete=\\S+`, name: `storage.other.command.completion.viml` },
        { begin: `(aug(roup)?)`, end: `(augroup\\sEND|$)`, name: `support.function.augroup.viml` },
      ],
    },
    comment: {
      patterns: [{ begin: `((\\s+)?""")`, end: `^(?!")`, name: `comment.block.documentation.viml` }, {
        match: `^"\\svim:.*`,
        ...
        patterns: [{ match: `\\{\\{\\{\\d?$`, name: `comment.line.foldmarker.viml` }, {
          match: `\\}\\}\\}\\d?`,
    ...
    constant: {
      patterns: [{ match: `\\b(true|false)\\b`, name: `constant.language.boolean.viml` }, {
        match: `\\b([0-9]+)\\b`,
        name: `constant.numeric.viml`,
      }],
    },
    entity: { patterns: [{ match: `(([absg]\\:)?[a-zA-Z0-9_#.]{2,})\\b(?=\\()`, name: `entity.name.function.viml` }] },
    keyword: {
      patterns: [
        { match: `\\b(if|while|for|return|au(g|group)|else(if|)?|do|in)\\b`, name: `keyword.control.viml` },
        { match: `\\b(end|endif|endfor|endwhile)\\s|$`, name: `keyword.control.viml` },
        { match: `\\b(break|continue|try|catch|endtry|finally|finish|throw|range)\\b`, name: `keyword.control.viml` },
        { match: `\\b(fun|func|function|endfunction|endfunc)\\b`, name: `keyword.function.viml` },
        { match: `\\b(normal|silent)\\b`, name: `keyword.other.viml` },
        { include: `#operators` },
      ],
    },
    map: {
      patterns: [{
        ...
        patterns: [
          { match: `(?<=:\\s)(.+)`, name: `constant.character.map.rhs.viml` },
          { match: `(?i:(bang|buffer|expr|nop|plug|sid|silent))`, name: `constant.character.map.special.viml` },
          { match: `(?i:([adcms]-\\w))`, name: `constant.character.map.key.viml` },
          { match: `(?i:(F[0-9]+))`, name: `constant.character.map.key.fn.viml` },
          {
            match: `(?i:(bs|bar|cr|del|down|esc|left|right|space|tab|up|leader))`,
            name: `constant.character.map.viml`,
          },
        ],
      }, { match: `(\\b([cinostvx]?(nore)?map)\\b)`, name: `storage.type.map.viml` }],
    },
    operators: {
      patterns: [
        { match: `([#+?!=~\\\\])`, name: `keyword.operator.viml` },
        { match: ` ([:\\-.]|[&|]{2})( |$)`, name: `keyword.operator.viml` },
        { match: `([.]{3})`, name: `keyword.operator.viml` },
        { match: `( [<>] )`, name: `keyword.operator.viml` },
        { match: `(>=)`, name: `keyword.operator.viml` },
      ],
    },
    ...
    storage: {
      patterns: [
        { match: `\\b(call|let|unlet)\\b`, name: `storage.viml` },
        { match: `\\b(abort|autocmd)\\b`, name: `storage.viml` },
        { match: `\\b(set(l|local)?)\\b`, name: `storage.viml` },
        { match: `\\b(com(mand)?)\\b`, name: `storage.viml` },
        { match: `\\b(color(scheme)?)\\b`, name: `storage.viml` },
        { match: `\\b(Plug|Plugin)\\b`, name: `storage.plugin.viml` },
      ],
    },
    strings: {
      patterns: [{ begin: `"`, end: `("|$)`, name: `string.quoted.double.viml`, patterns: [] }, {
        begin: `'`,
        end: `('|$)`,
    ...
    syntax: {
      patterns: [
        { match: `syn(tax)? case (ignore|match)`, name: `keyword.control.syntax.viml` },
        { match: `syn(tax)? (clear|enable|include|off|on|manual|sync)`, name: `keyword.control.syntax.viml` },
        {
          match: `\\b(contained|display|excludenl|fold|keepend|oneline|skipnl|skipwhite|transparent)\\b`,
          name: `keyword.other.syntax.viml`,
        },
        { match: `\\b(add|containedin|contains|matchgroup|nextgroup)\\=`, name: `keyword.other.syntax.viml` },
        {
          captures: { 1: { name: `keyword.other.syntax-range.viml` }, 3: { name: `string.regexp.viml` } },
          match: `((start|skip|end)\\=)(\\+\\S+\\+\\s)?`,
        },
        {
          captures: {
            0: { name: `support.type.syntax.viml` },
            1: { name: `storage.syntax.viml` },
            3: { name: `variable.other.syntax-scope.viml` },
            4: { name: `storage.modifier.syntax.viml` },
          },
        ...
        {
          captures: {
            1: { name: `storage.highlight.viml` },
            2: { name: `storage.modifier.syntax.viml` },
            3: { name: `support.function.highlight.viml` },
            4: { name: `variable.other.viml` },
            5: { name: `variable.other.viml` },
          },
    ...
    variable: {
      patterns: [{ match: `https?://\\S+`, name: `variable.other.link.viml` }, {
        match: `(?<=\\()([a-zA-Z]+)(?=\\))`,
        name: `variable.parameter.viml`,
      }, { match: `\\b([absgl]:[a-zA-Z0-9_.#]+)\\b(?!\\()`, name: `variable.other.viml` }],
    },
  },
  i = `source.viml`,
