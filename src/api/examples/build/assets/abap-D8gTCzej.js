var e = `ABAP`,
  ...
  a = [
    ...
    { match: `(?<![^\\s])##.*?(?=([\\.:,\\s]))`, name: `comment.line.pragma.abap` },
    {
      match: `(?i)(?<=(?:\\s|~|-))(?<=(?:->|=>))([a-z_\\/][a-z_0-9\\/]*)(?=\\s+(?:=|\\+=|-=|\\*=|\\/=|&&=|&=)\\s+)`,
      name: `variable.other.abap`,
    },
    { match: `\\b[0-9]+(\\b|\\.|,)`, name: `constant.numeric.abap` },
    { match: `(?ix)(^|\\s+)((PUBLIC|PRIVATE|PROTECTED)\\sSECTION)(?=\\s+|:|\\.)`, name: `storage.modifier.class.abap` },
    {
      begin: `(?<!\\\\)(\\|)(.*?)`,
    ...
    },
    {
      ...
      beginCaptures: { 1: { name: `storage.type.block.abap` }, 2: { name: `entity.name.type.block.abap` } },
      end: `\\s*\\.\\s*\\n?`,
    ...
    },
    {
      ...
      patterns: [
        ...
        { match: `(?ix)(?<=^|\\s)(FOR\\s+(HDB|LLANG))(?=\\s+|\\.)`, name: `storage.modifier.method.abap` },
        {
        ...
        { begin: `(?=[A-Za-z_][A-Za-z0-9_]*)`, end: `(?![A-Za-z0-9_])`, patterns: [{ include: `#generic_names` }] },
      ],
    },
    {
      begin: `(?ix)^\\s*(INTERFACE)\\s([a-z_\\/][a-z_0-9\\/]*)`,
      beginCaptures: { 1: { name: `storage.type.block.abap` }, 2: { name: `entity.name.type.abap` } },
      end: `\\s*\\.\\s*\\n?`,
      patterns: [{ match: `(?ix)(?<=^|\\s)(DEFERRED|PUBLIC)(?=\\s+|\\.)`, name: `storage.modifier.method.abap` }],
    },
    {
    ...
    { match: `(?i)(endclass|endmethod|endform|endinterface)`, name: `storage.type.block.end.abap` },
    { match: `(?i)(<[A-Za-z_][A-Za-z0-9_]*>)`, name: `variable.other.field.symbol.abap` },
    { include: `#keywords` },
...
adabas|as400|db2|db6|hdb|oracle|sybase|mssqlnt|pos_low|pos_high)(?=\\s|\\.|,)`,
    ...
    comparison_operator: {
      match:
        `(?i)(?<=\\s)(<|>|<\\=|>\\=|\\=|<>|eq|ne|lt|le|gt|ge|cs|cp|co|cn|ca|na|ns|np|byte-co|byte-cn|byte-ca|byte-na|byte-cs|byte-ns|o|z|m)(?=\\s)`,
      name: `keyword.control.simple.abap`,
    },
    control_keywords: {
      match: `(?ix)(^|\\s)(
...
endloop|endon|endtry|endwhile|if|loop|on|raise|try|while)(?=\\s|\\.|:)`,
    ...
    keywords: {
      patterns: [{ include: `#main_keywords` }, { include: `#text_symbols` }, { include: `#control_keywords` }, {
        include: `#keywords_followed_by_braces`,
      }],
    },
    keywords_followed_by_braces: {
      captures: { 1: { name: `keyword.control.simple.abap` }, 2: { name: `variable.other.abap` } },
      match: `(?ix)\\b(data|value|field-symbol|final|reference|resumable)\\((<?[a-z_\\/][a-z_0-9\\/]*>?)\\)`,
    },
    logical_operator: { match: `(?i)(?<=\\s)(not|or|and)(?=\\s)`, name: `keyword.control.simple.abap` },
    main_keywords: {
      match: `(?ix)(?<=^|\\s)(
abap-source|
abstract|
...
zone
    ...
    operators: {
      patterns: [{ include: `#other_operator` }, { include: `#arithmetic_operator` }, {
    ...
    reserved_names: { match: `(?ix)(?<=\\s)(me|super)(?=\\s|\\.|,|->)`, name: `constant.language.abap` },
    sql_functions: {
      match: `(?ix)(?<=\\s)(
...
weekday
    ...
    system_fields: {
      captures: { 1: { name: `variable.language.abap` }, 2: { name: `variable.language.abap` } },
      match:
    ...
    text_symbols: {
      captures: { 1: { name: `keyword.control.simple.abap` }, 2: { name: `constant.numeric.abap` } },
