var e = `PostCSS`,
  ...
  i = `postcss`,
  a = [
    { begin: `/\\*`, end: `\\*/`, name: `comment.block.postcss`, patterns: [{ include: `#comment-tag` }] },
    ...
    { match: `(?<=@import)\\s[\\w/.*-]+`, name: `string.quoted.double.css.postcss` },
    {
      begin: `@`,
      end:
    ...
    { include: `#operator` },
    { match: `[a-z-]+((?=:|#{))`, name: `support.type.property-name.css.postcss` },
  ...
  o = {
    ...
    'double-slash': { begin: `//`, end: `$`, name: `comment.line.postcss`, patterns: [{ include: `#comment-tag` }] },
    flag: { match: `!(important|default|optional|global)`, name: `keyword.other.important.css.postcss` },
    function: {
      match: `(?<=[\\s|\\(|,|:])(?!url|format|attr)[\\w-][\\w-]*(?=\\()`,
      name: `support.function.name.postcss`,
    },
    'function-content': { match: `(?<=url\\(|format\\(|attr\\().+?(?=\\))`, name: `string.quoted.double.css.postcss` },
    'function-content-var': { match: `(?<=var\\()[\\w-]+(?=\\))`, name: `variable.parameter.postcss` },
    interpolation: {
      begin: `#{`,
      end: `}`,
    ...
    numeric: { match: `(-|\\.)?[0-9]+(\\.[0-9]+)?`, name: `constant.numeric.css.postcss` },
    operator: {
      match: `\\+|\\s-\\s|\\s-(?=\\$)|(?<=\\()-(?=\\$)|\\s-(?=\\()|\\*|/|%|=|!|<|>|~`,
    ...
    'pseudo-class': { match: `:[a-z:-]+`, name: `entity.other.attribute-name.pseudo-class.css.postcss` },
    ...
    'rgb-value': { match: `(#)([0-9a-fA-F]{3}|[0-9a-fA-F]{6})\\b`, name: `constant.other.color.rgb-value.css.postcss` },
    'single-quoted': {
      begin: `'`,
      end: `'`,
    ...
    'variable-root-css': { match: `(?<!&)--[\\w-]+`, name: `variable.parameter.postcss` },
  },
  s = `source.css.postcss`,
  c = {
    displayName: e,
    fileTypes: t,
