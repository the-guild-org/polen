var e = `SAS`,
  ...
  a = [
    ...
    {
      begin: `\\b(?i:(data))\\s+`,
      beginCaptures: { 1: { name: `keyword.other.sas` } },
      comment: `Begins a DATA step and provides names for any output SAS data sets, views, or programs.`,
      end: `(;)`,
      patterns: [{ include: `#blockComment` }, { include: `#dataSet` }, {
        captures: { 1: { name: `keyword.other.sas` }, 2: { name: `keyword.other.sas` } },
  ...
  o = {
    ...
    dataSet: {
      patterns: [{
        begin: `((\\w+)\\.)?(\\w+)\\s?\\(`,
        beginCaptures: { 2: { name: `entity.name.class.libref.sas` }, 3: { name: `entity.name.class.dsname.sas` } },
        ...
        captures: { 2: { name: `entity.name.class.libref.sas` }, 3: { name: `entity.name.class.dsname.sas` } },
    ...
    dataSetOptions: {
      patterns: [{
        match:
          `(?<=\\s|\\(|\\))(?i:ALTER|BUFNO|BUFSIZE|CNTLLEV|COMPRESS|DLDMGACTION|ENCRYPT|ENCRYPTKEY|EXTENDOBSCOUNTER|GENMAX|GENNUM|INDEX|LABEL|OBSBUF|OUTREP|PW|PWREQ|READ|REPEMPTY|REPLACE|REUSE|ROLE|SORTEDBY|SPILL|TOBSNO|TYPE|WRITE|FILECLOSE|FIRSTOBS|IN|OBS|POINTOBS|WHERE|WHEREUP|IDXNAME|IDXWHERE|DROP|KEEP|RENAME)\\s?=`,
        name: `keyword.other.sas`,
      }],
    },
    macro: { patterns: [{ match: `(&+(?i:[a-z_]([a-z0-9_]+)?)(\\.+)?)\\b`, name: `variable.other.macro.sas` }] },
    operator: {
      patterns: [{ match: `([\\+\\-\\*\\^\\/])`, name: `keyword.operator.arithmetic.sas` }, {
        match: `\\b(?i:(eq|ne|gt|lt|ge|le|in|not|&|and|or|min|max))\\b`,
        name: `keyword.operator.comparison.sas`,
      }, { match: `([¬<>^~]?=(:)?|>|<|\\||!|¦|¬|^|~|<>|><|\\|\\|)`, name: `keyword.operator.sas` }],
    },
    quote: {
      patterns: [{
        begin: `(?<!%)(')`,
      ...
      }, { begin: `(")`, comment: `double quoted string block`, end: `(")([bx])?`, name: `string.quoted.double.sas` }],
    },
    starComment: {
      ...
      }, { begin: `^[\\s%]*\\*`, end: `;`, name: `comment.line.start.sas` }],
    },
  },
  s = `source.sas`,
  c = {
