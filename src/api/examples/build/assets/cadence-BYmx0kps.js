  ...
  r = {
    ...
    comments: {
      patterns: [
        ...
        {
          begin: `/\\*\\*(?!/)`,
          beginCaptures: { 0: { name: `punctuation.definition.comment.begin.cadence` } },
          end: `\\*/`,
          endCaptures: { 0: { name: `punctuation.definition.comment.end.cadence` } },
        ...
        { match: `\\*/`, name: `invalid.illegal.unexpected-end-of-block-comment.cadence` },
        {
          begin: `(^[ \\t]+)?(?=//)`,
          beginCaptures: { 1: { name: `punctuation.whitespace.comment.leading.cadence` } },
          end: `(?!\\G)`,
          patterns: [{
            begin: `///`,
            beginCaptures: { 0: { name: `punctuation.definition.comment.cadence` } },
            end: `^`,
      ...
      repository: { nested: { begin: `/\\*`, end: `\\*/`, patterns: [{ include: `#nested` }] } },
    },
    composite: {
      begin:
        `\\b((?:(?:struct|resource|contract)(?:\\s+interface)?)|transaction|enum)\\s+([\\p{L}_][\\p{L}_\\p{N}\\p{M}]*)`,
      beginCaptures: { 1: { name: `storage.type.$1.cadence` }, 2: { name: `entity.name.type.$1.cadence` } },
      end: `(?<=\\})`,
      name: `meta.definition.type.composite.cadence`,
      patterns: [{ include: `#comments` }, { include: `#conformance-clause` }, {
        begin: `\\{`,
        beginCaptures: { 0: { name: `punctuation.definition.type.begin.cadence` } },
        end: `\\}`,
    ...
    declarations: {
      patterns: [{ include: `#var-let-declaration` }, { include: `#function` }, { include: `#initializer` }],
    },
    event: {
      begin: `\\b(event)\\b\\s+([\\p{L}_][\\p{L}_\\p{N}\\p{M}]*)\\s*`,
      beginCaptures: { 1: { name: `storage.type.event.cadence` }, 2: { name: `entity.name.type.event.cadence` } },
      end: `(?<=\\))|$`,
    ...
    expressions: {
      patterns: [{ include: `#comments` }, { include: `#function-call-expression` }, { include: `#literals` }, {
        include: `#operators`,
      }, { include: `#language-variables` }],
    },
    function: {
      begin: `\\b(fun)\\b\\s+([\\p{L}_][\\p{L}_\\p{N}\\p{M}]*)\\s*`,
      beginCaptures: { 1: { name: `storage.type.function.cadence` }, 2: { name: `entity.name.function.cadence` } },
      end: `(?<=\\})|$`,
      name: `meta.definition.function.cadence`,
      patterns: [{ include: `#comments` }, { include: `#parameter-clause` }, { include: `#function-result` }, {
        begin: `(\\{)`,
    ...
    keywords: {
      patterns: [
        { match: `(?<!\\.)\\b(?:if|else|switch|case|default)\\b`, name: `keyword.control.branch.cadence` },
        { match: `(?<!\\.)\\b(?:return|continue|break)\\b`, name: `keyword.control.transfer.cadence` },
        { match: `(?<!\\.)\\b(?:while|for|in)\\b`, name: `keyword.control.loop.cadence` },
        { match: `(?<!\\.)\\b(?:pre|post|prepare|execute|create|destroy|emit)\\b`, name: `keyword.other.cadence` },
        {
          match: `(?<!\\.)\\b(?:private|pub(?:\\(set\\))?|access\\((?:self|contract|account|all)\\))\\b`,
          name: `keyword.other.declaration-specifier.accessibility.cadence`,
        },
        { match: `\\b(?:init|destroy)\\b`, name: `storage.type.function.cadence` },
        { match: `(?<!\\.)\\b(?:import|from)\\b`, name: `keyword.control.import.cadence` },
      ],
    },
    'language-variables': { patterns: [{ match: `\\b(self)\\b`, name: `variable.language.cadence` }] },
    literals: {
      patterns: [{ include: `#boolean` }, { include: `#numeric` }, { include: `#string` }, {
        match: `\\bnil\\b`,
        name: `constant.language.nil.cadence`,
      }],
      repository: {
        boolean: { match: `\\b(true|false)\\b`, name: `constant.language.boolean.cadence` },
        numeric: {
          patterns: [{ include: `#binary` }, { include: `#octal` }, { include: `#decimal` }, {
            include: `#hexadecimal`,
          }],
          repository: {
            binary: {
        ...
        string: {
          patterns: [{
            begin: `"`,
            beginCaptures: { 0: { name: `punctuation.definition.string.begin.cadence` } },
            end: `"`,
            endCaptures: { 0: { name: `punctuation.definition.string.end.cadence` } },
          ...
          repository: {
            'string-guts': {
              patterns: [{ match: `\\\\[0\\\\tnr"']`, name: `constant.character.escape.cadence` }, {
    ...
    'parameter-clause': {
      begin: `(\\()`,
      beginCaptures: { 1: { name: `punctuation.definition.parameters.begin.cadence` } },
      end: `(\\))`,
    ...
    'parameter-list': {
      patterns: [{
        captures: { 1: { name: `entity.name.function.cadence` }, 2: { name: `variable.parameter.function.cadence` } },
        ...
        captures: { 1: { name: `variable.parameter.function.cadence` }, 2: { name: `entity.name.function.cadence` } },
        ...
        patterns: [{ include: `#type` }, { match: `:`, name: `invalid.illegal.extra-colon-in-parameter-list.cadence` }],
      }],
    },
    type: {
      patterns: [{ include: `#comments` }, {
