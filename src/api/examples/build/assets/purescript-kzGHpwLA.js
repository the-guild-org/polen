var e = `PureScript`,
  ...
  r = [
    ...
    },
    {
      ...
      patterns: [{ include: `#comments` }, { include: `#module_name` }, { include: `#module_exports` }, {
        match: `[a-z]+`,
    ...
    },
    {
      ...
      patterns: [{ include: `#double_colon` }, { include: `#kind_signature` }],
    },
    {
      ...
      patterns: [{ include: `#double_colon` }, { include: `#type_signature` }],
    },
    {
      begin: `^\\s*\\b(import)(?!')\\b`,
      beginCaptures: { 1: { name: `keyword.other.purescript` } },
      ...
      patterns: [{ include: `#module_name` }, { include: `#module_exports` }, {
    ...
    },
    {
      ...
      beginCaptures: {
        ...
        3: { name: `meta.type-signature.purescript`, patterns: [{ include: `#type_signature` }] },
      },
      end: `^(?!\\1[ \\t]|[ \\t]*$)`,
      name: `meta.declaration.type.data.purescript`,
      patterns: [
        { include: `#comments` },
        { captures: { 0: { name: `keyword.operator.assignment.purescript` } }, match: `=` },
        {
          captures: {
            1: { patterns: [{ include: `#data_ctor` }] },
            2: { name: `meta.type-signature.purescript`, patterns: [{ include: `#type_signature` }] },
          },
          match:
            `(?:(?:\\b([\\p{Lu}\\p{Lt}][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*(?:\\.[\\p{Lu}\\p{Lt}][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*)*)\\s+)(?:(?<ctorArgs>(?:(?:[\\p{Lu}\\p{Lt}][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*(?:\\.[\\p{Lu}\\p{Lt}][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*)*|(?:[\\p{Lu}\\p{Lt}][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*(?:\\.[\\p{Lu}\\p{Lt}][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*)*\\.)?[\\p{Ll}_][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*|(?:(?:[\\w()'→⇒\\[\\],]|->|=>)+\\s*)+))(?:\\s*(?:\\s+)\\s*\\g<ctorArgs>)?)?))`,
        },
        { captures: { 0: { name: `punctuation.separator.pipe.purescript` } }, match: `\\|` },
    ...
    { match: `^\\s*\\b(data|type|newtype)(?!')\\b`, name: `storage.type.purescript` },
    { match: `\\b(do|ado|if|then|else|case|of|let|in)(?!('|\\s*(:|=)))\\b`, name: `keyword.control.purescript` },
    { match: `\\b(?<!\\$)0(x|X)[0-9a-fA-F]+\\b(?!\\$)`, name: `constant.numeric.hex.purescript` },
    {
      captures: {
...
(?:\\b[0-9]+\\b(?!\\.))
    ...
    { match: `\\b(true|false)\\b`, name: `constant.language.boolean.purescript` },
    { match: `\\b(([0-9]+_?)*[0-9]+|0([xX][0-9a-fA-F]+|[oO][0-7]+))\\b`, name: `constant.numeric.purescript` },
    {
      begin: `"""`,
      beginCaptures: { 0: { name: `punctuation.definition.string.begin.purescript` } },
      end: `"""`,
      endCaptures: { 0: { name: `punctuation.definition.string.end.purescript` } },
    ...
    },
    {
      ...
      patterns: [{ include: `#characters` }, {
        begin: `\\\\\\s`,
        beginCaptures: { 0: { name: `markup.other.escape.newline.begin.purescript` } },
        end: `\\\\`,
        endCaptures: { 0: { name: `markup.other.escape.newline.end.purescript` } },
        patterns: [{ match: `\\S+`, name: `invalid.illegal.character-not-allowed-here.purescript` }],
      }],
    },
    ...
    { match: `[\\p{S}\\p{P}&&[^(),;\\[\\]\`{}_"']]+`, name: `keyword.operator.purescript` },
  ...
  i = {
    block_comment: {
      patterns: [{
        ...
        begin: `\\{-\\s*\\|`,
        beginCaptures: { 0: { name: `punctuation.definition.comment.documentation.purescript` } },
        end: `-\\}`,
        endCaptures: { 0: { name: `punctuation.definition.comment.documentation.purescript` } },
        name: `comment.block.documentation.purescript`,
        patterns: [{ include: `#block_comment` }],
      }, {
        applyEndPatternLast: 1,
        begin: `\\{-`,
        beginCaptures: { 0: { name: `punctuation.definition.comment.purescript` } },
        end: `-\\}`,
    ...
    comments: {
      patterns: [{
        begin: `(^[ \\t]+)?(?=--+\\s+\\|)`,
        beginCaptures: { 1: { name: `punctuation.whitespace.comment.leading.purescript` } },
        end: `(?!\\G)`,
        patterns: [{
          begin: `(--+)\\s+(\\|)`,
          beginCaptures: {
            1: { name: `punctuation.definition.comment.purescript` },
            2: { name: `punctuation.definition.comment.documentation.purescript` },
          },
          end: `\\n`,
    ...
    double_colon: { patterns: [{ match: `(?:::|∷)`, name: `keyword.other.double-colon.purescript` }] },
    ...
    record_field_declaration: {
      patterns: [{
        ...
        patterns: [{ include: `#type_signature` }, { include: `#record_types` }],
      }],
    },
    record_types: {
      patterns: [{
        begin: `\\{`,
        beginCaptures: { 0: { name: `keyword.operator.type.record.begin.purescript` } },
        end: `\\}`,
        endCaptures: { 0: { name: `keyword.operator.type.record.end.purescript` } },
        name: `meta.type.record.purescript`,
        patterns: [{ match: `,`, name: `punctuation.separator.comma.purescript` }, {
    ...
    type_signature: {
      patterns: [
        ...
        },
        {
          ...
          match:
            `((?:(?:([\\p{Lu}\\p{Lt}][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*(?:\\.[\\p{Lu}\\p{Lt}][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*)*)\\s+)(?:(?<classConstraint>(?:[\\p{Lu}\\p{Lt}][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*(?:\\.[\\p{Lu}\\p{Lt}][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*)*|(?:[\\p{Lu}\\p{Lt}][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*(?:\\.[\\p{Lu}\\p{Lt}][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*)*\\.)?[\\p{Ll}_][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*)(?:\\s*(?:\\s+)\\s*\\g<classConstraint>)?))))\\s*(=>|<=|⇐|⇒)`,
          name: `meta.class-constraints.purescript`,
        },
        { match: `->|→`, name: `keyword.other.arrow.purescript` },
        { match: `=>|⇒`, name: `keyword.other.big-arrow.purescript` },
