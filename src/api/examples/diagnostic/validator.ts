 ...
 */
const parseDocumentSafe = (content: string): E.Either<DocumentNode, GraphQLError> => {
  return E.try({
    try: () => parse(content),
...
export const validateExamples = (
  ...
  A.forEach(examples, (example) => {
    if (example.document._tag === 'DocumentVersioned') {
      ...
      A.forEach(documentVersions, (version) => {
...
const formatValidationMessage = (
  exampleId: string,
  version: Version.Version | undefined,
  errors: A.NonEmptyReadonlyArray<GraphQLError>,
): string => {
  const versionStr = version ? ` (${Version.encodeSync(version)})` : ''
  ...
  const firstError = A.headNonEmpty(errors)
